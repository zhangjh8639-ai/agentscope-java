...
import io.agentscope.builder.runtime.marketplace.MarketSkillSummary;
import io.agentscope.builder.runtime.marketplace.UserMarketplacePersistence;
 ...
 *
 * <p>This deliberately differs from claw's controller: there is no global admin tier, and the
 * path does not include a {@code {userId}} segment — the active principal is always the owner.
 * Cross-user access (even by name) returns 404, not 403, to avoid leaking marketplace ids across
 * accounts.
 */
@RestController
...
public class MarketplacesController {
    ...
    public Mono<MarketplaceSummary> updateMarketplace(
        ...
        return Mono.fromCallable(
                () -> {
                    ...
                    // Body's id is ignored on PUT; path id wins so a typo can't move the entry.
    ...
    public Mono<List<MarketSkillBrief>> listSkills(@PathVariable String id, Authentication auth) {
        ...
        return Mono.fromCallable(
                () -> {
                    BuilderMarketplace mp = requireRegistered(userId, id);
    ...
    private BuilderMarketplace requireRegistered(String userId, String id) {
        validateId(id);
