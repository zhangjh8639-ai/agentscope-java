 ...
 *   <li><b>channel-default</b> — {@link ChannelConfig#defaultAgentId()}
 *   <li><b>requested</b> — {@link InboundMessage#requestedAgentId()} caller hint
 *   <li><b>global-default</b> — {@code globalDefaultAgentId}
 * </ol>
 *
 * <p>Note: explicit bindings and {@code channel.defaultAgentId} always take precedence over the
 * caller hint, so administrators can pin routes regardless of what a UI or external caller asks
 * for. The hint is only consulted when neither bindings nor a channel-level default exist.
 *
 ...
 * "agentId"} so that {@link io.agentscope.claw2.runtime.gateway.HarnessGateway#run} can pick the correct
...
public final class ChannelRouter {
     ...
     *     io.agentscope.claw2.runtime.gateway.Gateway#bindMainAgent}
     */
    ...
    public RouteResult resolveRoute(ChannelConfig config, InboundMessage msg) {
        ...
        } else if (msg.requestedAgentId() != null && !msg.requestedAgentId().isBlank()) {
            agentId = msg.requestedAgentId();
            matchedBy = "requested";
        } else {
        ...
        MsgContext context = buildContext(msg, effectiveScope, agentId);
    ...
    private MsgContext buildContext(InboundMessage msg, DmScope dmScope, String agentId) {
        ...
        if (peer.kind().isThread()) {
            ...
            return new MsgContext(channel, msg.guild(), parentRoom, peer.id(), null, extra);
        } else if (peer.kind().isDirect()) {
            return buildDmContext(channel, peer.id(), msg.accountId(), dmScope, extra);
        } else {
            // Non-DM channel / group: room = peer id, group = guild
            return new MsgContext(channel, msg.guild(), peer.id(), null, null, extra);
        }
    }
    ...
    private OutboundAddress buildOutboundAddress(InboundMessage msg) {
        // Use peer.key() ("<kind>:<id>") so outbound adapters can recover the peer kind
        // (DM vs group, etc.) when picking the right send endpoint.
        String to = msg.channelId() + ":" + msg.peer().key();
