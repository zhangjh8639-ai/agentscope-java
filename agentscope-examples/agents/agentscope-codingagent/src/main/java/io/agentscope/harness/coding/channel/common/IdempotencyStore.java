 ...
 */
package io.agentscope.harness.coding.channel.common;
...
/**
 * Bounded per-channel idempotency store for inbound webhook / stream events. Providers commonly
 * retry the same message id under failure; this store de-duplicates by {@code msgId} or
 * {@code event_id}.
 *
 * <p>Internal map is bounded to {@link #maxEntries} — when full, the oldest entries are evicted to
 * make room. Entries are also lazily expired after {@link #ttlMillis}.
 */
public final class IdempotencyStore {
    ...
    /** Default 5-minute TTL, 10k entries. */
