...
/**
 * Parses a DingTalk Stream bot-message payload (subscribed topic {@code
 * /v1.0/im/bot/messages/get}) into an {@link InboundMessage}.
 *
 * <p>For MVP only {@code msgtype=text} is mapped. Other inbound types are returned as {@link
 * Optional#empty()} so the caller can ack the event without dispatching to the agent.
 *
 ...
 * </ul>
 *
 * <p>Group messages typically arrive only when the bot is @-mentioned (DingTalk's bot SDK dispatches
 * mention-triggered events only); the leading {@code @bot} text is preserved by the platform in
 * {@code text.content}.
 */
