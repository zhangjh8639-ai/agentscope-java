...
export default function AgentSettingsForm({ agent }: { agent: AgentDefinition }) {
  ...
  return (
    <div style={S.page}>
      {isGlobal && (
        <div style={S.banner}>
          Built-in (global) agents are read-only from the UI. Fork the agent to create an editable
          custom copy, or edit <code>.agentscope/agentscope.json</code> on the server to change them.
        </div>
      )}
      ...
      <div style={{ ...S.card, marginTop: 24 }}>
        ...
        <div style={S.row}>
          <label style={S.fieldLabel}>Scope</label>
          <div style={S.meta}>{agent.scope}</div>
        </div>
        {agent.forkOf && (
          <div style={S.row}>
            <label style={S.fieldLabel}>Fork of</label>
